...
# Run prepare_metadata_with_ftp.sh, which calls this script.
    ...
    with open(raw_metadata_in) as inf:
        for line in inf:
            ...
            for fastq_ftp in fastq_ftps.split(";"):
                ...
                data.append([plant, date, filename, fastq_ftp, "1" if enriched else "0"])

    data.sort()

    with open(parsed_metadata_out, "w") as outf:
        outf.write("\t".join(["filename", "date", "fastq_ftp", "plant", "is_enriched"]) + "\n")
        for plant, date, filename, fastq_ftp, is_enriched in data:
            outf.write("\t".join([filename, date, fastq_ftp, plant, is_enriched]) + "\n")
